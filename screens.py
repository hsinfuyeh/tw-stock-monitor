...
TAB_LABEL = {
    ...
    "screen": "體質篩選", "pos": "動能", "rev": "月營收", "sue": "營收驚奇",
}
...
SCREENS = [
    ...
    ("rev", "月營收年增",
     ...
     "看中段比看榜首有意義。", "fact"),
    ("sue", "營收驚奇（公布後漂移）",
     "月營收<b>超出公司自己常態</b>最多的一群 —— 不是「成長最快」，是「比預期好最多」。"
...
def run_screen(panel, key, limit=40):
    ...
    if key == "sue":
        ...
        if "days_since" in r.columns:
            r = r[r["days_since"] <= 45]
        return r.sort_values("sue", ascending=False).head(limit), "sue", "營收驚奇"
    if key == "pos":
...
for _k in ['sue']:
    NOTES[_k] = (
        ...
        "營收驚奇問的是「<b>比自己的常態好多少</b>」。一家連續三年年增 60% 的公司，"
        "這個月公布 40%，在年增率排行上仍然名列前茅 —— 但那是負驚奇，"
        "實測顯示這種股票接下來會弱。<br><br>"
