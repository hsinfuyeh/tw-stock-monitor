...
ORDER_NOTE = (
    ...
    "減掉當日整池平均）：<br><br>"
    "<b>價格位置 + 月營收年增 + 優於預期（現在用的）+0.58%，t = 4.77</b><br>"
    "「優於預期」單獨 +0.56%（t = 5.35）、價格位置 + 月營收年增 +0.45%（t = 3.57）<br>"
    ...
    "t &gt; 3.0</b>。加入「優於預期」之前，前半只有 t = 1.99（勉強擦邊），"
    "這是這次改動最大的收穫。<br><br>"
    "<b>但仍要注意</b>：+0.58% <b>小於 0.6% 的來回交易成本</b>，"
    "所以這個排序不能當成 10 日換股策略；它的用途是<b>決定你先研究哪幾檔</b>。"
    "若要真的跨過成本，看「月營收優於預期」榜並把持有期拉到 40-60 個交易日 —— "
    "那是本系統唯一測得出足夠空間的組合。")
