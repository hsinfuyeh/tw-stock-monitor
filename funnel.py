...
ORDER_NOTE = (
    ...
    "減掉當日整池平均）：<br><br>"
    "<b>價格位置 + 月營收年增 + 營收驚奇（現在用的）+0.58%，t = 4.77</b><br>"
    "營收驚奇單獨 +0.56%（t = 5.35）、價格位置 + 月營收年增 +0.45%（t = 3.57）<br>"
    ...
    "<br><br>"
    "<b>前後兩半分別是 +0.41%（t = 3.17）與 +0.74%（t = 3.75），兩半都通過"
    "t &gt; 3.0</b>。加入營收驚奇之前，前半只有 t = 1.99（勉強擦邊），"
    "這是這次改動最大的收穫。<br><br>"
    "<b>但仍要注意</b>：+0.58% <b>小於 0.6% 的來回交易成本</b>，"
    "所以這個排序不能當成 10 日換股策略；它的用途是<b>決定你先研究哪幾檔</b>。"
    "若要真的跨過成本，看「營收驚奇」榜並把持有期拉到 40-60 個交易日 —— "
    "那是本系統唯一測得出足夠空間的組合。")
...
def rank_candidates(d):
    ...
    現在用三個在池內仍有鑑別力的東西等權合成：價格位置、月營收年增、營收驚奇。
    各自先做當日橫斷面標準化再平均，所以量綱不同不影響。

    為什麼不用「營收驚奇單獨」—— 它的 t 值其實最高（+0.56%，t = 5.35，
